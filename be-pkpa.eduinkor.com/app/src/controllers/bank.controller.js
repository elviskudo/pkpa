...
const createData = async (req, res, next) => {
  try {
    const pData = {
      ...
      va_code: req.body.va_code,
      is_active: req.body.is_active,
      order: req.body.order,
    };

    if (req.files.image_url) {
      pData.image_url = req.files.image_url[0].location;
    }
...
const getAllData = async (req, res, next) => {
  ...
  // }
  try {
    const data = await dbModel.findAll({ where: { is_active: true }, attributes: { exclude: ['createdAt', 'updatedAt', 'deletedAt'] } });
