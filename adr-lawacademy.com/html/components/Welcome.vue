        ...
        <v-card tile max-width="500" width="100%">
          <div class="tw-w-11/12 mx-auto" style="font-family: 'Open Sans';">
            ...
            <v-card-text v-if="statusExam || loggedInUser.pass_exam_pmb">
              ...
              <div class="tw-flex tw-justify-center tw-items-center tw-text-center tw-font-bold border tw-h-24 tw-w-full tw-mt-3" style="background-color: #FFFAF2; border: 1px solid #FF9900; border-radius: 6px;">
                <div style="font-family: 'Raleway';">
                  ...
                  <span class="tw-block tw-text-3xl" style="color: #FF9900;">Rp2.500.000</span>
                </div>
              </div>
            </v-card-text>
            ...
            <v-card-actions v-else class="px-4">
              <!-- <nuxt-link :to="to" style="color: #fff; background-color: #FF9900;" class="tw-flex tw-justify-center tw-items-center tw-w-full tw-mx-auto rounded tw-py-2 tw-mb-5 hover:tw-bg-orange-600 tw-font-semibold" v-if="loggedInUser.status">
                {{ oke }}
              </nuxt-link> -->
              <a href="javascript:;" style="color: #fff; background-color: #FF9900;" class="tw-flex tw-justify-center tw-items-center tw-w-full tw-mx-auto rounded tw-py-2 tw-mb-5 hover:tw-bg-orange-600 tw-font-semibold" @click.stop="createInvoice">
                {{ oke }}
              </a>
            </v-card-actions>
          </div>
  ...
  async fetch () {
    ...
    if (this.isAuthenticated) {
      ...
      // } else { // (this.statusExam || this.loggedInUser.pass_exam_pmb) {
        ...
        this.subtitle = 'Sebagai Peserta di Pendidikan Khusus Profesi Advokat Atmasasmita, Dodi & Rekan Law Firm'
        this.color = 'blue'
    ...
    async createInvoice() {
      ...
      try {
        const body = {
          ...
          userID: this.loggedInUser.id || null,
          externalID: `payment-${this.loggedInUser.id}` || null,
          amount: this.$config.xenditAmount,
          payerEmail: this.loggedInUser.email || '',
          description: 'Biaya Pendaftaran',
        }
        const headers = {
          'Authorization': `Basic ${Buffer.from(this.$config.xenditPublicKey).toString('base64')}`
        }

        const response = await this.$axios.$post('payment/invoice/create', body, headers)
