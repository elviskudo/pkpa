    ...
    <v-card>
      <div class="tw-flex tw-flex-row tw-items-center tw-justify-between tw-px-3 tw-mt-5">
        ...
        <button
          ...
          @click="add()"
        >
          <span class="tw-text-blue-500 tw-font-bold">+ Tambah Topik</span>
        </button>
      </div>
      <v-data-table
        v-model="selected"
  ...
  methods: {
    async initialize () {
      ...
      if (this.loggedInUser.university_id) {
        ...
        res = await this.$axios.$get(`bank-topic/?university_id=${this.university_id}`)
      } else if (this.university_id) {
        res = await this.$axios.$get(`bank-topic/?university_id=${this.university_id}`)
      } else {
        res = await this.$axios.$get('bank-topic')
      }
      this.items = res.data.data
    },

    viewItem (item) {
      this.$router.push(`/dashboard/admin/topic/view/${item.id}`)
    },
    editItem (item) {
      this.$router.push(`/dashboard/admin/topic/edit/${item.id}`)
    },
    ...
    async deleteItemConfirm () {
      ...
      await this.$axios.delete(`bank-topic/${this.editedItem.id}`)
      this.closeDelete()
    },
    ...
    add () {
      this.$router.push('/dashboard/admin/topic/add')
    }
  }
}
</script>
