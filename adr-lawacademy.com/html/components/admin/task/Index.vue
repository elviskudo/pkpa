      ...
      >
        <template #top>
          ...
          <!-- <v-toolbar
            flat
          >
            <v-toolbar-title>Manajemen Tugas</v-toolbar-title>
            <v-divider
              class="tw-mx-4"
              inset
              vertical
...
export default {
  ...
  methods: {
    async initialize () {
      const res = await this.$axios.$get('answer-task')
    ...
    viewItem (item) {
      this.$router.push(`/dashboard/admin/task/view/${item.id}`)
    },
    editItem (item) {
      this.$router.push(`/dashboard/admin/task/edit/${item.id}`)
    },
