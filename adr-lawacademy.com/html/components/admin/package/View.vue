            ...
            <div class="tw-px-12">
              <div class="md:tw-flex md:tw-items-center tw-mb-6">
                ...
                <div class="md:tw-w-2/3">
                  <select v-model="university_id" name="state" class="tw-form-select tw-w-full" disabled @change="getphaseuniversitas">
                    <option selected disabled value="">
                      Pilih
                    </option>
                    <option v-for="unv in univercities" :key="unv.id" :value="unv.id" :selected="univercities == university_id">
                      {{ unv.name }}
                    </option>
                  </select>
                </div>
              ...
              <div class="md:tw-flex md:tw-items-center tw-mb-6">
                ...
                <div class="md:tw-w-2/3">
                  <select v-model="phase_university_id" name="state" class="tw-form-select tw-w-full" disabled>
                    <option selected disabled value="">
                      Pilih
                    </option>
              ...
              <div class="md:tw-flex md:tw-items-center tw-mb-6">
                ...
                <div class="md:tw-w-2/3">
                  <select v-model="matakuliah_id" name="state" class="tw-form-select tw-w-full" disabled>
                    <option selected disabled value="">
                      Pilih
                    </option>
              ...
              <div class="md:tw-flex md:tw-items-center tw-mb-6">
                ...
                <div class="md:tw-w-2/3">
                  <select v-model="category_id" name="state" class="tw-form-select tw-w-full" disabled>
                    <option selected disabled value="">
                      Pilih
                    </option>
                      ...
                      <v-expansion-panel-content>
                        <div class="tw-grid tw-grid-cols-3 tw-py-2">
                          ...
                          <div class="tw-col-span-2">
                            <select v-model="item.id" name="state" class="tw-form-select tw-w-full" disabled @change="getcontentTopic(index, item.id)">
                              <option selected disabled value="null">
                                Pilih
                              </option>
                              <option v-for="phs in topics" :key="phs.id" :value="phs.id" :selected="phs === item.id">
                                {{ phs.name }}
                              </option>
                            </select>
                          </div>
                        ...
                        <div v-for="(ct,index2) in item.contentTopics" :key="index2" :value="index2">
                          <div v-if="ct.id !== null" class="tw-grid tw-grid-cols-9 tw-gap-3">
                            <div class="tw-col-span-3">
            ...
            >
              ...
              <div v-if="kuncikonten">
                <div class="md:tw-flex md:tw-items-center tw-mb-6">
                  ...
                  <div class="md:tw-w-2/3">
                    <select v-model="package_id" name="package" class="tw-form-select tw-w-full" disabled>
                      <option selected disabled value="null">
                        Pilih
                      </option>
                      <option v-for="p in packages" :key="p.id" :value="p.id" :selected="p == package_id">
                        {{ p.name }}
                      </option>
                    </select>
                  </div>
...
export default {
  ...
  async mounted () {
    const res = await this.$axios.$get(`course/package-detail/${this.$route.params.id}`)
    this.itemsTopic = res.data
    this.matakuliah_id = res.data[0].course_id
    this.university_id = res.data[0].university_id
    this.category_id = res.data[0].content_category_id
    this.phase_university_id = res.data[0].phase_id
    this.is_publish = res.data[0].is_publish
    this.package_id = res.data[0].predecessor
    if (this.package_id !== null) {
      this.is_active = true
      this.kuncikonten = true
    }

    const mt = await this.$axios.$get('bank-course')
    ...
    const phase = await this.$axios.$get(`phase-university/?university_id=${this.university_id === null ? 0 : this.university_id}`)
    ...
    const topics = await this.$axios.$get('bank-topic')
    this.topics = topics.data.data.map(obj => ({ id: obj.id, name: obj.name }))

    const resPre = await this.$axios.$get(`course/list-predecessor/?phaseId=${this.phase_university_id}&contentCategoryId=${this.category_id}`)
    this.packages = resPre.data
  },
  methods: {
    async submit () {
      const data = {
        ...
        content_key: this.is_active,
        itemstp: JSON.stringify(this.itemsTopic)
      }
      try {
        const res = await this.$axios.$put(`course/${this.$route.params.id}`, data)
        if (!res.error) {
          this.$swal({
            icon: 'success',
            title: 'Sukses..!!',
