    ...
    <v-card>
      <div class="tw-flex tw-flex-row tw-items-center tw-justify-between tw-px-3 tw-mt-5">
        ...
        <button
          ...
          @click="add()"
        >
          <span class="tw-text-blue-500 tw-font-bold">+ Tambah Kategori</span>
        </button>
      </div>
      <v-data-table
        :search="search"
        ...
        class="elevation-1"
      >
        <template #[`item.event_datetime`]="{ item }">
          <span>{{ $moment(item.event_datetime).format('DD MMM YYYY HH:mm') }}</span>
        </template>
        <template #top>
          ...
          <!-- <v-toolbar
            flat
          >
            <v-toolbar-title>Jadwal Ujian Akhir</v-toolbar-title>
            <v-divider
              class="tw-mx-4"
              inset
              vertical
...
export default {
  data: () => ({
    ...
    headers: [
      ...
      { text: 'Gelombang', value: 'phase_name' },
      { text: 'Judul Acara', value: 'title' },
      { text: 'Waktu', value: 'event_datetime' },
  ...
  methods: {
    async initialize () {
      const res = await this.$axios.$get('live-streaming/?type=final_exam')
      this.data = res.data.data
    },
    viewItem (item) {
      this.$router.push(`/dashboard/admin/live-streaming/final-exam/view/${item.id}`)
    },
    editItem (item) {
      this.$router.push(`/dashboard/admin/live-streaming/final-exam/edit/${item.id}`)
    },
    ...
    add () {
      this.$router.push('/dashboard/admin/live-streaming/final-exam/add')
    }
  }
}
</script>
