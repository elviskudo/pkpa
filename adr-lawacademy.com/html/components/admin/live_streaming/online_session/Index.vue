      ...
      >
        ...
        <template #top>
          ...
          <!-- <v-toolbar
            flat
          >
            <v-toolbar-title>Jadwal Sesi Online</v-toolbar-title>
            <v-divider
              class="tw-mx-4"
              inset
              vertical
...
export default {
  ...
  methods: {
    async initialize () {
      const res = await this.$axios.$get('live-streaming/?type=online_session')
      this.data = res.data.data
    },
    viewItem (item) {
      this.$router.push(`/dashboard/admin/live-streaming/online-session/view/${item.id}`)
    },
    editItem (item) {
      this.$router.push(`/dashboard/admin/live-streaming/online-session/edit/${item.id}`)
    },
    ...
    add () {
      this.$router.push('/dashboard/admin/live-streaming/online-session/add')
    }
  }
}
</script>
