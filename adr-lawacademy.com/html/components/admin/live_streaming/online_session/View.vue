          ...
          <v-form>
            <div class="tw-grid tw-grid-cols-3 tw-px-3 tw-py-3">
              ...
              <div class="tw-col-span-2">
                <select v-model="university_id" class="tw-w-2/3 tw-form-select" disabled @change="getPhaseUniversity">
            ...
            <div class="tw-grid tw-grid-cols-3 tw-px-3 tw-py-3">
              ...
              <div class="tw-col-span-2">
                <select v-model="phase_id" name="phase" class="tw-form-select tw-w-2/3" disabled @change="changePhase">
                  <option selected disabled value="null">
                    Pilih
                  </option>
            ...
            <div class="tw-grid tw-grid-cols-3 tw-px-3 tw-py-3">
              ...
              <div class="tw-col-span-2">
                <select v-model="course_id" name="course" class="tw-form-select tw-w-2/3" disabled>
                  <option selected disabled value="null">
                    Pilih
                  </option>
            ...
            <div class="tw-grid tw-grid-cols-3 tw-px-3 tw-py-3">
              ...
              <div class="tw-col-span-2 ">
                <input v-model="name" type="text" class="tw-form-input tw-w-2/3" disabled>
              </div>
            </div>
            <div class="tw-grid tw-grid-cols-3 tw-px-3 tw-py-3">
              <div class="tw-col-span-3">
              ...
              <div class="tw-col-span-2 ">
                <input v-model="event_datetime" type="datetime-local" class="tw-form-input tw-w-2/3" disabled>
              </div>
            </div>
            <div class="tw-grid tw-grid-cols-3 tw-px-3 tw-py-3">
              <div class="tw-col-span-3">
              ...
              <div class="tw-col-span-2 ">
                <input v-model="link" type="text" class="tw-form-input tw-w-2/3" disabled>
              </div>
            </div>
            <v-divider />
...
export default {
  ...
  async mounted () {
    const res = await this.$axios.$get(`live-streaming/${this.$route.params.id}`)
    this.university_id = res.data[0].university_id
    this.phase_id = res.data[0].phase_id
    this.course_id = res.data[0].course_id
    this.name = res.data[0].name
    this.event_datetime = this.$moment(res.data[0].event_datetime).format().slice(0, 16)
    ...
    const res2 = await this.$axios.$get(`phase-university/?university_id=${this.university_id}`)
    this.phases = res2.data.data.map(obj => ({ id: obj.id, name: obj.name, register_period_start: obj.register_period_start, register_period_end: obj.register_period_end, learning_period_start: obj.learning_period_start, learning_period_end: obj.learning_period_end }))

    const res3 = await this.$axios.$get(`course/?phase_id=${this.phase_id}`)
    this.courses = res3.data.data.map(item => ({ id: item.id, name: item.name }))
  },
  methods: {
    async submit () {
      const data = {
      ...
      try {
        const res = await this.$axios.$put(`live-streaming/${this.$route.params.id}`, data)
        if (!res.error) {
          this.$swal({
            icon: 'success',
            title: 'Sukses..!!',
          ...
          this.$router.push('/dashboard/admin/live-streaming/online-session')
        } else {
          this.$swal({
            icon: 'error',
            title: 'Oops...',
