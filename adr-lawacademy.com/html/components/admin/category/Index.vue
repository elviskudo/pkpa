...
export default {
  data: () => ({
    ...
    headers: [
      ...
      { text: 'Judul Kategori', value: 'name' },
      { text: 'Tipe', value: 'is_active' },
  ...
  methods: {
    ...
    editItem (item) {
      this.$router.push(`/dashboard/admin/category/edit/${item.id}`)
    },
    viewItem (item) {
      this.$router.push(`/dashboard/admin/category/view/${item.id}`)
    },
    close () {
      this.dialog = false
      this.$nextTick(() => {
    ...
    add () {
      this.$router.push('/dashboard/admin/category/add')
    },
    deleteItem (item) {
    ...
    async deleteItemConfirm () {
      await this.$axios.$delete(`bank-content-category/${this.editedItem.id}`)
      this.initialize()
      this.closeDelete()
    }
  }
