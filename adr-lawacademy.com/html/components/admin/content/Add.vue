              ...
              <div class="tw-grid tw-grid-cols-3 tw-px-3 tw-py-3">
                <div class="tw-col-span-3">
                  ...
                  <input
                    v-model="link_url"
                    type="text"
                    class="tw-form-input tw-w-2/3 tw-placeholder-gray-300"
                    placeholder="contoh : https://www.youtube.com/embed/qnP8XRQ3hNA"
                  >
                </div>
              </div>
            </div>
                      ...
                      <v-expansion-panel-content>
                        ...
                        <div class="tw-grid tw-grid-cols-3 tw-py-2">
                          <div class="tw-col-span-3">
                            ...
                            <div v-for="(answer, index2) in question.answers" :key="index2" class="tw-flex tw-flex-row tw-items-center">
                              <input
...
export default {
  ...
  methods: {
    async submitSettingQuiz () {
      ...
      try {
        const res = await this.$axios.$post('bank-content-topic', data)
        if (!res.error) {
    ...
    async submit () {
      ...
      try {
        const res = await this.$axios.$post('bank-content-topic', formData, config)
        if (!res.error) {
          this.$swal({
            icon: 'success',
            title: 'Sukses..!!',
    ...
    async submitContentQuiz (index) {
      ...
      try {
        const res = await this.$axios.$post('bank-content-question', formData, config)
        if (!res.error) {
          this.$swal({
            icon: 'success',
            title: 'Sukses..!!',
    ...
    changeremidial () {
      if (parseInt(this.remidial_type, 10) === 12) {
