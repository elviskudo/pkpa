...
export default {
  ...
  async fetch () {
    const universities = await this.$axios.$get(`/university/${this.$route.params.id}`)
    this.universities = universities.data
    this.nameUniv = universities.data.name
    this.kode = universities.data.slug
    this.gambar = universities.data.image_url
    this.logo = universities.data.logo_url
    this.visi = universities.data.vision
    this.misi = universities.data.mission
    this.tujuan = universities.data.goal
    this.polapokok = universities.data.motto
    this.id = universities.data.id
  },
  methods: {
    async submit () {
      ...
      try {
        const res = await this.$axios.$put(`/university/${this.id}`, formData, config)
        if (!res.error) {
          this.$swal({
            icon: 'success',
            title: 'Sukses..!!',
    ...
    fbtnshowlogo () {
      this.showInputLogo = true
      this.showlogo = false
    },
    fbtnbatallogo () {
      this.showInputLogo = false
      this.showlogo = true
    },
    fbtnshowInputImg () {
