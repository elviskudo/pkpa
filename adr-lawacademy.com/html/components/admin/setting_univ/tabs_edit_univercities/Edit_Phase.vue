...
export default {
  ...
  methods: {
    async initialize () {
      const res = await this.$axios.$get(`/phase-university/?university_id=${this.$route.params.id}`)
      // console.log(res)
      this.data = res.data.data
      const no = 1
      this.data.forEach((_, i) => (this.data[i].no = no + i))
    },
    viewItem (item) {
      this.$router.push(`/dashboard/admin/setting-univ/tab-view-univ-phase/${item.id}`)
    },
    editItem (item) {
      this.$router.push(`/dashboard/admin/setting-univ/tab-edit-univ-phase/${item.id}`)
    },
    ...
    add () {
      this.$router.push('/dashboard/admin/setting-univ/tab-add-univ-phase/add')
    }
  }
}
</script>
